...
module.exports = {
  config: {
    ...
    hasPrefix: true,
    aliases: ["monga"],
    description: "Monga Image Generator",
    usage: "monga <prompt> | <model> | <control>",
  ...
  run: async function ({ api, event, args }) {
    try {
      ...
      if (commandParts.length !== 3) {
        await api.sendMessage("⚠️ | Invalid command format. Please use: monga <prompt> | <model> | <control>", event.threadID, event.messageID);
        return;
      }
      ...
      console.error('Error processing monga command:', error);
      await api.sendMessage("⚠️ | Error processing monga command. Please try again later.", event.threadID, event.messageID);
    }
  }
};
